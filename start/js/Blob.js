...
BlobWorld = new function() {
  ...
  function createBlob( position ) {
    ...
    // Play start sound
    // uncomment the following line to start sound when the blob is created
  ...
  function splitBlob( blob ) {
    if( blob.quality > 8 ) {
      ...
      // play start sound for split increase playback rate
      // uncomment the following lines to play the start sound when the blob is split and increase the playback rate
  ...
  function mergeBlobs( blobA, blobB ) {
    ...
    if( t - blobs[blobA].lastSplitTime > 500 && t - blobs[blobB].lastSplitTime > 500 ) {
      ...
      // play merge sound, decrease playback rate
      // uncomment the following lines to play the merge sound and decrease the playback rate
      //SoundMixer.startSound.playbackRate -= 1;
      //SoundMixer.play( SoundMixer.mergeSound );
